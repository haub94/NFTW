<script lang="ts">
  ...
  function setTestimonialData() {
    for (
      let customerIndex = 0;
      customerIndex < CUSTOMER_DATA.length;
      customerIndex++
    ) {
      if (CUSTOMER_DATA[customerIndex].testimonial !== EMPTY_STRING) {
        ...
        for (let imageIndex = 0; imageIndex < IMAGES.length; imageIndex++) {
          if (
            IMAGES[imageIndex].name === CUSTOMER_DATA[customerIndex].testimonial
          ) {
            currentImage = selectImageFromDb(
              CUSTOMER_DATA[customerIndex].testimonial,
              IMAGES
            ) as string[];
          }
        }
        ...
        for (
          let ratingIndex = 0;
          ratingIndex < RATING_DATA.length;
          ratingIndex++
        ) {
          if (
            CUSTOMER_DATA[customerIndex].testimonial ===
            RATING_DATA[ratingIndex].testimonial
          ) {
        ...
        testimonialData.push({
          name:
            CUSTOMER_DATA[customerIndex].firstName +
            " " +
            CUSTOMER_DATA[customerIndex].lastName,
          bookedDestination: getDestinationFromString(
            CUSTOMER_DATA[customerIndex].journeyConfig
          ),
          bookedPurpose: getPurposeFromString(
            CUSTOMER_DATA[customerIndex].journeyConfig
          ),
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Gewinnen Sie Einblicke in Einsatz-Szenarien der Event Rallye"
  />
</svelte:head>
    ...
    <div class="w-9/12 h-auto mx-auto overflow-hidden">
      ...
      >
        {#each testimonialData as person}
          {#if person.name !== ""}
            <!--quick and dirty to kickoff the first (index=0), empty object-->
            <swiper-slide>
              <TestimonialCard
                name={person.name}
